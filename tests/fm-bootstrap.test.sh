...
test_unknown_backend_reports_invalid_configuration() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  assert_contains "$out" "BACKEND_INVALID: bogus (known: tmux herdr zellij orca cmux stream)" \
...
deck shared efforts are accepted^{"rules":[{"when":"deck low","use":{"harness":"deck","effort":"low"}},{"when":"deck medium","use":{"harness":"deck","effort":"medium"}},{"when":"deck high","use":{"harness":"deck","effort":"high"}},{"when":"deck xhigh","use":{"harness":"deck","effort":"xhigh"}},{"when":"deck max","use":{"harness":"deck","effort":"max"}}]}^empty^
unsupported deck ultra effort is flagged^{"rules":[{"when":"deck ultra","use":{"harness":"deck","effort":"ultra"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - invalid effort: deck:ultra
