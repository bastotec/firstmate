...
| Turn end | Before touching the task's turn-end notification, the driver requires a new `done`, `needs-decision`, `blocked`, `failed`, or `working` status line and safely appends `failed: deck turn ended without a status line (<event>)` when one is absent; status reads and fallback appends reject symlinks, non-regular files, and hard links. |
