...
| Delivery | The submit path snapshots the task's semantic busy sequence before Enter and confirms Deck only when a newer record reads `state=busy source=deck-wrapper`; rendered model output is never delivery evidence. The driver still renders a transient `⛵ deck working - ctrl+c to stop` row, then replaces it with the final turn output and the `❯` prompt. |
