...
| Exit | `/quit`, one Enter; the driver records session-end and exits, leaving the pane's shell. Relaunch cleanup signals the driver and Deck itself, waits boundedly for the isolated process group to exit, and escalates survivors to KILL before replacement. Deck 0.1.0 exits immediately on TERM, so this cleanup can interrupt an active in-process tool. |
