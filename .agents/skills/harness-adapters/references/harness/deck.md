...
| Per-turn bounds | `--max-turns` 200 and `--deadline-secs` 3600 by default (`FM_DECK_MAX_TURNS`, `FM_DECK_DEADLINE_SECS`); Deck's own defaults are sized for one question; a native deadline resumes only when the last event is an unfinished `no-mistakes axi run`, `gh run watch`, or `gh pr checks --watch` tool call, with at most 3 continuations per task by default (`FM_DECK_DEADLINE_ROLLOVERS`), while provider waits and every other deadline fail the turn normally. |
...
| Exit | `/quit`, one Enter; the driver records session-end and exits, leaving the pane's shell; relaunch cleanup signals both the driver and Deck itself, waits for Deck to stop after its active tool and exit, then proves the isolated process group gone before replacement, with a bounded timeout escalating the remaining group to KILL. |
