...
- `VERSION_UNREADABLE: <tool> (installed build; requires semantic version >=<floor>; upgrade: <command>)` - the essential tool exists, but its version output cannot prove that it satisfies the floor, so do not dispatch work that depends on it and do not describe it as absent.
  Tell the captain that the installed build's compatibility is unknown, request consent for the printed upgrade command, then run `bin/fm-bootstrap.sh install <approved tool>` and rerun session start to confirm a semantic version at or above the floor.
...
- `HOME_ROUTE: this home ships firstmate's own changes from <checkout>, and <reason> - ...` - the checkout this home delivers firstmate's OWN changes from, or the validation pipeline registration saved for it, has an origin that lands on this host, so a validated change to firstmate itself is pushed there and no pull request is ever opened; nothing errors and nothing is lost, which is why the absent PR is the only symptom.
  Homes seeded as standalone clones before the seed-time gate existed are the population this line reaches, so treat it as a real finding about this home rather than a transient.
  Do not repair it in passing: repointing a live home's `origin` at the fork and refreshing its saved validation registration is its own migration with its own unlanded-work checks.
  Report it to the captain as work that needs deciding, and until it is done, deliver firstmate-repo changes from a home whose route is the fork.
...
- `CREW_DISPATCH: invalid config/account-slots.json - <reason>` or `CREW_DISPATCH: invalid account slot routing - <reason>` - the optional account-slot registry, or a dispatch rule's `accountSlots` reference, failed the same low-cost validation; treat it exactly as above.
  An invalid registry short-circuits the rest of the dispatch validation, so correct it and rerun before concluding the dispatch file itself is clean.
...
- `BACKLOG_RECONCILE: code-root <file> is not this home's <file>; ...` - a tasks-axi write addressed the code root instead of this home, so the queue has already forked and either copy may hold rows the other lacks; [`docs/configuration.md`](../../../docs/configuration.md) ("Backlog backend") owns why.
  The check only fires for a home whose `FM_HOME` is the code root, so every file it can name belongs to the home it is diagnosing and the remedy can never touch another home's queue.
...
- `SECONDMATE_LIVENESS: secondmate <id>: posture mismatch: live agent pid <pid> lacks the configured Claude permission flag '<flag>' (<where>); reported only, not relaunched` - the secondmate's agent is running, but not on the permission posture `config/claude-permission-mode` selects; a restarted Herdr server resuming its session is the known cause ([`docs/herdr-backend.md`](../../../docs/herdr-backend.md) "Restart and liveness behavior").
  Tell the captain which secondmate is on the wrong posture; replacing its agent is the captain's call, and nothing here stops or relaunches it.
