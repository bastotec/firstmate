...
- `HOME_ROUTE: this home ships firstmate's own changes from <checkout>, and <reason> - ...` - the checkout this home delivers firstmate's OWN changes from has an origin that lands on this host, so a validated change to firstmate itself is pushed there and no pull request is ever opened; nothing errors and nothing is lost, which is why the absent PR is the only symptom.
  ...
  Do not repair it in passing: repointing a live home's `origin` at the fork, and keeping its local code root reachable under the `code-root` remote name, is its own migration with its own unlanded-work checks.
