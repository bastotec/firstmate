...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then tmux; tmux is the verified reference backend (docs/tmux-backend.md), herdr has its own required CI lane (docs/herdr-backend.md), while zellij, orca, cmux, and stream remain experimental with no dedicated real-backend CI lane (docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md, docs/stream-backend.md) - herdr and cmux can also be selected by runtime auto-detection, zellij, orca, and stream never are (always explicit), and codex-app is not accepted; see docs/codex-app-backend.md; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
...
A worker rebases only while its branch is still its own to rewrite, and nothing but the pipeline owns that branch during an active validation run, so a change that lands mid-run waits for the run to finish rather than becoming a steer into it.
Once that run has finished, or once the worker has reported its final `done:`, getting the branch current is yours to own by steering or relaunching the worker, never by running a rebase in its worktree yourself.
