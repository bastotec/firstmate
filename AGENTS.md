...
config/stream-hub config/stream-token config/stream-hub-tokens  stream backend hub address and credentials: the fleet hub's base URL, this home's own client token, and, on the host that runs the hub, the class-scoped "<classes>:<token>" lines it serves; LOCAL, gitignored, never committed, and not inherited; see docs/stream-backend.md
