...
state/               runtime records and signals; gitignored
  ...
  <id>.stooddown      marks a task the captain intentionally stopped (epoch then reason) so its idle stale and stall re-rings are provably noise; written by bin/fm-wake-gate.sh stand-down, cleared by resume, and read by the fail-open wake gate
