...
A worker rebases only while its branch is still its own to rewrite, so a change that lands during its active validation run or after its final `done:` is yours to get current by steering or relaunching the worker, never by running a rebase in its worktree yourself.
