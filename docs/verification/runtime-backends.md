...
- cmux confirms from a `workspace list --json` listing that omits the workspace.
  ...
  That also covers the expected-label path, which answers one false for several different reasons: a listing that ran and omits the workspace, or shows it under another title, decides; a listing that failed is unconfirmed; and a workspace the listing still shows under this task's own title is live, so it is closed and confirmed rather than reported gone.
