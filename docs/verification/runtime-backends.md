...
- Zellij confirms from an `action list-panes --json` listing that omits the pane, from a `list-sessions` run that omits the session, and from an expected task label no live tab in that session carries any more.
  A listing that does not run or does not parse is unconfirmed, including an absent zellij CLI: one classifier owns that answer and quotes the failing listing's own first line, so the refusal names its cause.
...
- stream confirms only from the endpoint's own agent, either a record it closed after watching the worker exit or a kill the hub reports it took.
  ...
  One qualified form of that answer does confirm absence: a hub that passes `GET /v1/health`, reports an uptime past its own `state_max_age_secs` re-registration window, and lists no such endpoint. Uptime comes from the hub's `uptime_secs`, never from subtracting `started_at` against a local clock a remote caller does not share.
