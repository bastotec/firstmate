...
The pending-close record teardown already stages before destructive cleanup carries that intent as a `mode=retain` line, so a cleanup interrupted after its endpoint gate proved the worker stopped replays the retention at the next session start through the same record, validator, and lock as an ordinary close and never closes the row; an interruption before that gate leaves the record carrying the unproved stop, and session start then holds it for a rerun rather than replaying anything; if the captain answers before replay, `answer` validates that record and copies any supported retained pull request or report into the row before closing it, after which replay retires the record.
