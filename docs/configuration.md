...
This section is the single owner of the accepted backend list and of each backend's status - spawn capability, verification and CI coverage, auto-detected versus explicit-only selection, secondmate support, and which guide owns its details - and every other document points here instead of restating any part of it.
