...
Because the step decides what to run, `test.instructions` points it at the suites under `tests/` - which drive the real scripts against real worktrees and real processes, with PATH-shimmed terminal and harness binaries - names `bin/fm-test-run.sh` as the authority on which lane or family a suite belongs to, and rules out both the additional rig-building a lifecycle change would otherwise invite and the `live-harness-optin` family, which launches real harnesses and real terminal servers.
