...
Because the step decides what to run, `test.instructions` points it at the suites under `tests/` - which drive the real scripts against real worktrees and real processes, with PATH-shimmed terminal and harness binaries - and rules out the additional rig-building a lifecycle change would otherwise invite.
