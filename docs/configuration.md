...
`fm-spawn.sh` accepts `tmux`, `herdr`, `zellij`, `orca`, `cmux`, and `stream` for ship and scout tasks; `backend=orca`, `backend=cmux`, and `backend=stream` all still refuse `--secondmate` until secondmate launch semantics are designed for each.
...
A stream spawn additionally requires `python3`, `curl`, and `jq`, and version-gates against the hub's advertised protocol, refusing loudly when a dependency is missing, the hub is unreachable, the configured token is refused, or the protocol does not match.
...
Because the step decides what to run, `test.instructions` points it at the suites under `tests/` - which drive the real scripts against real worktrees and real processes, with PATH-shimmed terminal and harness binaries - names `bin/fm-test-run.sh` as the authority on which suites exist, gives `FM_LIVE=0 bin/fm-test-run.sh tests/<subject>.test.sh` as the per-subject invocation so the live-gated suites self-skip instead of launching real harnesses and real terminal servers, and rules out the additional rig-building a lifecycle change would otherwise invite.
`commands.test` and `test.instructions` decide what the gate runs, so no-mistakes honors them only from the default-branch copy of `.no-mistakes.yaml`; a pushed branch cannot change its own validation rules.
...
FM_BACKEND=             # optional runtime backend override for new spawns; tmux/herdr/zellij/orca/cmux/stream support ship/scout spawns, codex-app is not accepted
