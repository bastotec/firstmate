...
Deck secondmates use the driver-owned [Deck supervision protocol](supervision-protocols/deck.md); the main primary remains out of scope.
...
| `config/voice-hud-decoder` | `FM_VOICE_HUD_DECODER` | The spoken HUD's local wake-word decoder command, run through the shell; absent means gate-only and the HUD never wakes. |
