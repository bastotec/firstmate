...
Because the step decides what to run, `test.instructions` points it at the portable suites under `tests/` - which drive the real scripts against real worktrees and real processes, with PATH-shimmed terminal and harness binaries - and rules out both the additional rig-building a lifecycle change would otherwise invite and the `*-live-e2e` guards, which launch real harnesses and real terminal servers.
