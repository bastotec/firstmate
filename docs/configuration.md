...
This section is the single owner of the accepted backend list, of each backend's spawn capability, of auto-detected versus explicit-only selection, of secondmate support, of which backends are session providers only versus which own their own worktree lifecycle, and of which guide owns its details, and every other document points here instead of restating those axes.
