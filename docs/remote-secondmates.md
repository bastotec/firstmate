...
- The required remote tool set is `git`, `jq`, `herdr`, compatible `tasks-axi`, `treehouse`, and at least one of `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, or `deck`; macOS additionally requires `lsof` so the doctor and guard can prove which process owns the session socket.
