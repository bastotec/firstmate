...
- The required remote tool set is `git`, `jq`, `herdr`, compatible `tasks-axi`, `treehouse`, and at least one of `deck`, `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, or `kimi`; Deck additionally requires `python3`, and macOS additionally requires `lsof` so the doctor and guard can prove which process owns the session socket.
