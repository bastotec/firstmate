...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  A successful row grant transfers ownership of exactly the currently branch-eligible rows; its actor-local drain folds repeated kind/key rows to the latest row for presentation while the whole granted set remains acknowledgeable, and a check-kind triggering close (merge-confirmation polls, Relay mentions, credential/auth failures, and every other legitimately main-only class) is never offered even when other rows are eligible, no acceptor (extension absent, legacy away daemon flag, branch broken) keeps today's wake-to-main path for that close, and watcher-failure alarms always go to main because only main can repair the watcher cycle.
...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  After wake rows are claimed, a branch prompt counts as handled only when `fm_branch_report` appends one durable outcome for every distinct presented row before that prompt settles; repeated same-kind/key queue rows fold to the latest presented row, duplicate reports for that row do not cover another presented row, and a settled provider error or incomplete settled prompt releases the grant and rejects delivery ownership back to the watcher.
  The branch bash boundary refuses the drain's acknowledgement command until every presented row has that durable coverage, so a partial report cannot consume the rest of the grant before settlement checks it.
  Once coverage is complete, the extension uses that same actor acknowledgement boundary before settling; if the provider fails before the branch runs the acknowledgement itself, the extension runs the exact command observed from that prompt's drain and refuses settlement when no unique command was observed, so a second presenting drain cannot absorb later rows.
  `fm_branch_report` binds every claimed signal or stale wake sequence to the task that row resolves to (a signal row by its status-log key, a stale row through the task record naming that endpoint), including when a heartbeat review drains those rows; only a genuine heartbeat row is task-unscoped, and the review may append additional unbound task or fleet outcomes after all claimed rows have their own reports.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  ...
  The branch recomputes eligibility immediately before prompting the branch to drain and publishes the exact eligible ownership set to `state/.branch-eligible-rows` through `writeEligibleRowsSnapshot`; report coverage is keyed separately to the actor-local deduplicated rows that drain presents.
...
The branch can also run on a cheaper model and a shallower reasoning effort than main, both pinned with the Pi `/supervision-model` command, and the model pin may be an ordered fallback chain that moves to the next model when one fails; [configuration.md](configuration.md#pi-supervision-branch-model-and-effort-configsupervision-branch-model-configsupervision-branch-effort) owns those pins' operator-facing schema and unpinned behavior.
