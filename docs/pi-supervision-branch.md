...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  While a signal or stale prompt is open, `fm_branch_report` binds each report to the claimed wake sequence and accepts only the task that row resolves to (a signal row by its status-log key, a stale row through the task record naming that endpoint); a report for any other task id, `fleet` included, is refused before the store is touched, so a task remembered from an earlier wake cannot become a delivered outcome, while a heartbeat review remains unscoped by task.
