...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  Once coverage is complete, the extension uses that same actor acknowledgement boundary before settling, including when the provider fails before the branch runs the printed command itself; releasing a grant never substitutes for consuming it.
