...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  Once coverage is complete, the extension uses that same actor acknowledgement boundary before settling; every fallback runs the exact command observed from that prompt's drain and refuses settlement when no unique command was observed, whether the prompt ended normally or with a provider error, so a second presenting drain cannot absorb later rows.
