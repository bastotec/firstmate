...
Runtime auto-detection is innermost-first: `$TMUX` wins over `HERDR_ENV=1`, which wins over cmux's primary `CMUX_WORKSPACE_ID` marker and documented fallback signals; auto-detected herdr or cmux prints a one-time opt-out notice, auto-detected tmux stays silent, and zellij, orca, and stream are never auto-detected (only explicit selection).
...
tmux, zellij, orca, cmux, and stream expose no native busy primitive at all, so a task on those backends is classified purely from its adapter's own lifecycle record.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, stream has a verified classifier but accepts no secondmate spawns, Zellij remains unverified, and Orca and cmux do not support secondmate spawns either.
...
stream is experimental and selected only explicitly: its "session host" is the fleet's own central hub rather than a third-party multiplexer, each task's pseudoterminal is owned by a thin agent on the machine that runs it, Treehouse remains its worktree provider, and `--secondmate` spawns are refused.
...
Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, cmux, and stream tasks, while Orca creates its own worktrees for `backend=orca`.
...
Fleet state lives in each task's session-provider backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca/stream when explicitly selected), no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
