...
`tests/fm-watch-triage.test.sh` pins the first-sight alarm, call-lifecycle throttle identity, fail-open backlog cases, and declared-wait ladder reset.
...
Deck secondmates keep the arm wrapper as a child of their persistent driver and route watcher output through durable-inbox turns; the main Deck primary remains unsupported.
...
Composer classification has one shared owner, `bin/fm-composer-lib.sh`: tmux, herdr, Zellij, Orca, cmux, and stream contribute only a screen capture plus declarative styled, cursor, identity, and row capabilities, while the shared classifier owns every shape and the `empty`/`pending`/`pending-unproven`/`unknown` verdict.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux, Herdr, and stream have verified agent-process classifiers, while Zellij, Orca, and cmux remain unverified on that axis; [`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns which backends accept a secondmate spawn.
...
[`zellij-backend.md`](zellij-backend.md) owns Zellij's current setup and limits, while [`verification/runtime-backends.md`](verification/runtime-backends.md#zellij) owns active empirical evidence.
...
Orca owns both worktree and terminal lifecycle, records `orca_worktree_id=` and `terminal=`, and removes worktrees through `orca worktree rm` only after the usual firstmate teardown checks pass.
...
cmux is GUI-first and macOS-only; [`cmux-backend.md`](cmux-backend.md) owns its current setup and limits, while [`verification/runtime-backends.md`](verification/runtime-backends.md#cmux) owns active source and live evidence.
cmux's container shape is one workspace per task with one surface, no per-home container split; workspace titles are scoped by the active home label plus a short hash of the resolved `FM_ROOT` path.
...
Codex App support is recorded in [`codex-app-backend.md`](codex-app-backend.md), which owns that boundary.
