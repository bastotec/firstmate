...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux, Herdr, and stream have verified agent-process classifiers for secondmate recovery, while Zellij, Orca, and cmux remain unverified on that axis.
