...
#                 "TANGLE: <remediation>",
#                 "HOME_ROUTE: <why this home's firstmate-repo changes would
#                 never become a pull request>",
...
# fm_backend_required_tools (bin/fm-backend.sh). So a herdr/zellij/cmux/stream
# home is never told tmux is missing, and only orca drops treehouse. A backend value with
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","grok","kimi","cursor","agy","deck","muse","rovo","omp"] | index($h);
    def effort_ok($h; $m; $e):
      if $e == null then true
      elif ($e | type) != "string" then false
      elif $e == "ultra" then (($h == "pi" or $h == "pi-signed") and (($m | type) == "string") and ($m | startswith("codex-native/")) and ($m | length) > 13)
      elif $h == "claude" or $h == "deck" then (["low","medium","high","xhigh","max"] | index($e))
...
detect_local_config() {
  ...
  # Delivery-route check: the checkout this home ships firstmate's own changes
  # from must reach a real remote (see fm-home-route-lib.sh). A home seeded as a
